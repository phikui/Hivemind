...
public class Executable implements java.io.Serializable {
	...
	public long execute(HashMap<String, Expression> variables)
		...
		// clear outputs
		variables.put(InputOutput.getOutput().getProperty("move_direction"),
				new Number(-1));
		variables.put(InputOutput.getOutput().getProperty("atack_strength"),
				new Number(0));
		while (!execution_stack.isEmpty()) {
			...
			if (currentExecutionTime > maxExecutionTime
					|| variables.size() > maxVariableMapSize) {
				throw new ExecuteException(
						"Exceeded maximum execution time of "
								+ (maxExecutionTime / 1000) + " seconds");
			}
			try {

				execution_stack.pop()
						.evaluate(variables, execution_stack, this);
			} catch (ValidExit e) {
				currentExecutionTime = System.currentTimeMillis() - startTime;
